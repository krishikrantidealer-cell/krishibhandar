  ...
  Widget build(BuildContext context) {
    return BlocBuilder<CartBloc, CartState>(
      builder: (context, cartState) {
        ...
        final cartImages = cartState.uniqueImages;

        if (itemCount == 0) return const SizedBox.shrink();

        final limitedImages =
            cartImages.length > 3 ? cartImages.sublist(cartImages.length - 3) : cartImages;
                ...
                child: Row(
                  ...
                  children: [
                    if (limitedImages.isNotEmpty) ...[
                      SizedBox(
                        width: (34.0 + (math.min(limitedImages.length, 3) - 1) * 20.0),
                        height: 34,
                        child: Stack(
                          children: List.generate(
                            math.min(limitedImages.length, 3),
                            (index) => Positioned(
                              left: index * 20.0,
                              child: Container(
                                width: 34,
